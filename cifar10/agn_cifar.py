...
def main():
    ...
    result_ls,losses = agn_attack( encoder, target_model, decoder, disc, trainloader, testloader, smallloader,
     encoder_opt, target_opt,  decoder_opt, disc_opt, device, config, save_model_path, seed)

    res = np.array([ result_ls, losses])
    np.save(results_path / f"agn_{seed}", res)
