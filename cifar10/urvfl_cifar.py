...
def main():
    ...
    disc = CLSDiscriminator(10) 
    disc_opt = torch.optim.Adam(disc.parameters(), lr=1e-4)

    

    if config['save']:
        pretrain_model(encoder, decoder, shadow_model, smallloader, testloader, encoder_opt, decoder_opt, shadow_opt, device, config, save_model_path, seed)
    else:
        save_seed = 222 ## the seed of the saved model
    ...
    dis_ls, cosine_ls, result_mse, result_psnr, result_ssim = cls_pretrain(target_model, encoder, decoder, shadow_model, disc, trainloader, smallloader, testloader, 
         target_opt, disc_opt, device, config, save_model_path, seed)
 
    
    res = np.array([ dis_ls, cosine_ls, result_mse, result_psnr, result_ssim])
    np.save(results_path / f"urvfl_{seed}", res)
