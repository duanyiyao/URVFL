...
def main():
    ...
    if config['vfl_save']:
        ...
        encoder.load_state_dict(torch.load(save_model_path+str(save_seed) +'vfl_enc.pt'))
        target_model.load_state_dict(torch.load(save_model_path+str(save_seed)+'vfl_target.pt'))
        server.load_state_dict(torch.load(save_model_path+str(save_seed)+'vfl_server.pt'))
        
    decoder_mse, enc_mse, enc_cos = gia_attack( server, encoder, target_model, shadow_model, shadow_opt, smallloader, testloader, device, 
               config)
    res = np.array([decoder_mse, enc_mse, enc_cos])
